...
exps_frames_paths = [f'/home/avic/OOD_Orientation_Generalization/exps{exp_iter}.csv' for exp_iter in range(1,4)]
...
for frame_path in exps_frames_paths:
    ...
    for exps_subset, is_scale in [(exps_frame[~exps_frame.scale], False)]:
        ...
        def generate_eval(_, exp):
            if is_scale:
                ...
                result = get_results(exp, 20, False)
                if result is None:
                    return
                exp, heatmap, _, image = result
                np.save(exp.eval_heatmap, heatmap)
