...
class RotationDataset:
    ...
    def setup_frames(self):
        ...
        if self.exp_data.num_fully_seen == 50:
            ...
            self.exp_data.training_frame = instances_frame[~instances_frame.index.isin(self.exp_data.full_validation_frame.df.index)].copy()
            self.exp_data.training_frame['weights'] = 1
            self.exp_data.partial_base_frame = pd.DataFrame()
            self.exp_data.partial_ood_frame = pd.DataFrame()
...
class _Dataset(Dataset):
    ...
    def get_img(self, idx):
        if self.preload_dataset:
            images = self.loaded_dataset[idx].float() / 256
        else:
